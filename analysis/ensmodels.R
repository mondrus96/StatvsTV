...
load("../classifiers/ADNI/FBS_cpDFC2.rda")
CVeval$Name = "cpd2"
...
df = CVeval
for(i in 2:10){
  load(paste0("../classifiers/ensADNI/", i, "ens.rda"))
  CVeval[cols_to_convert] = lapply(CVeval[cols_to_convert], as.numeric)
...
df = df[df$Model == "linsvm",] # focus on linear svm
...
df$ymin <- df$Accuracy - df$AccSE
df$ymax <- df$Accuracy + df$AccSE

# Plot Accuracy
png("EnsAccuracy.png", width = 10, height = 5, units = "in", res = 300, pointsize = 4)
...
# Plot Sensitivity
png("EnsSensitivity.png", width = 10, height = 5, units = "in", res = 300, pointsize = 4)
df$Name <- factor(df$Name, levels = df$Name)
...
# Plot Specificity
png("EnsSpecificity.png", width = 10, height = 5, units = "in", res = 300, pointsize = 4)
df$Name <- factor(df$Name, levels = df$Name)
...
# Plot F1
png("EnsF1.png", width = 10, height = 5, units = "in", res = 300, pointsize = 4)
df$Name <- factor(df$Name, levels = df$Name)
...
# Statistical tests
mod1 = df$Accuracy[df$Name == "top8"]
mod2 = df$Accuracy[df$Name == "cpd2"]
print(paste0("8ens > FBS_cpDFC2: ", prop.test(mod1*68, 68, mod2, "greater")$p.value)) # fail to reject null